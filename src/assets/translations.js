export const adminTranslations = [
  {
    id: 1,
    english: "ADMIN",
    chinese: "管理員",
  },
  {
    id: 2,
    english: "DOWNLOAD DATA",
    chinese: "下載資料",
  },
  {
    id: 2,
    english: "LOGOUT",
    chinese: "登出",
  },
  {
    id: 3,
    english: "Surveys Answered:",
    chinese: "已回答的調查:",
  },
  {
    id: 4,
    english: "Messsages",
    chinese: "訊息",
  },
];

export const contactUsTranslations = [
  {
    id: 1,
    english: "CONTACT US",
    chinese: "聯繫我們",
  },
  {
    id: 2,
    english: "Name is required!",
    chinese: "需要名字!",
  },
  {
    id: 3,
    english: "Invalid email format!",
    chinese: "無效的電子郵件格式!",
  },
  {
    id: 4,
    english: "Email is required!",
    chinese: "需要電子郵件!",
  },
  {
    id: 5,
    english: "Subject is required!",
    chinese: "需要主題!",
  },
  {
    id: 6,
    english: "Message is required!",
    chinese: "需要信息!",
  },
  {
    id: 7,
    english: "Name",
    chinese: "名字",
  },
  {
    id: 8,
    english: "Email",
    chinese: "電子郵件",
  },
  {
    id: 9,
    english: "Subject",
    chinese: "主題",
  },
  {
    id: 10,
    english: "Message",
    chinese: "信息",
  },
  {
    id: 11,
    english: "SUBMIT",
    chinese: "繳交",
  },
];

export const homeTranslations = [
  {
    id: 1,
    english: "INTRODUCTION",
    chinese: "簡介",
  },
  {
    id: 2,
    english: "START",
    chinese: "開始",
  },
  {
    id: 3,
    english:
      "This questionnaire primarily measures your happiness index during activity participation. It will use a 13-item Happiness Index Assessment Form to assess your current feelings on a scale of 1-10. It can help you understand your current physical and mental state and provide some feedback based on your condition.",
    chinese:
      "本問卷主要檢測你在參與活動當下的幸福指數，將採用13題幸福指數評估表以1-10分評估當下感受，能夠幫助你了解當下的身心狀況，並根據你的狀況給予一些回饋。",
  },
  {
    id: 4,
    english:
      "Welcome! Get to know more about our project. Take the test and get some personalized feedback.",
    chinese:
      "歡迎使用幸福生活網路,本網站提供幸福指 數評估表,幫助您在參與活動的過程中了解當下的身 心狀況,並給予一些回饋。",
  },
];

export const loginTranslations = [
  {
    id: 1,
    english: "ADMIN",
    chinese: "管理員",
  },
  {
    id: 2,
    english: "Username can't be empty!",
    chinese: "用戶名不能為空",
  },
  {
    id: 3,
    english: "Password can't be empty!",
    chinese: "密碼不能為空!",
  },
  {
    id: 4,
    english: "Username",
    chinese: "用戶名",
  },
  {
    id: 5,
    english: "Password",
    chinese: "密碼",
  },
  {
    id: 6,
    english: "SUBMIT",
    chinese: "繳交",
  },
  {
    id: 7,
    english: "Go Back Home",
    chinese: "返回首頁",
  },
];

export const surveyTranslations = [
  {
    id: 1,
    english: "Happiness Index Assessment",
    chinese: "幸福指數評估表",
  },
  {
    id: 2,
    english: "Use a scale of 1-10 to evaluate your level of feelings",
    chinese: "本量尺用1-10評估你的感受程度",
  },
  {
    id: 3,
    english: "Disagree",
    chinese: "非常不同意",
  },
  {
    id: 4,
    english: "Agree",
    chinese: "非常同意",
  },
  {
    id: 5,
    english: "What is your email address?",
    chinese:
      "想獲取相關健康促進活動資訊，可留下聯絡方式，我們將不定期通知活動。",
  },
  {
    id: 6,
    english: "Email",
    chinese: "電子郵件",
  },
  {
    id: 7,
    english: "What activity are you doing now?",
    chinese: "你認為今天的活動類型",
  },
  {
    id: 8,
    english: "SUBMIT",
    chinese: "繳交",
  },
  {
    id: 9,
    english:
      "Take our free happiness test in less than 5 minutes and discover more about yourself.",
    chinese:
      "本網站問卷過程約5分鐘，主要了解你當下的情緒狀態，所獲得的數據，將分數經過轉換後，給予相關的幸福活動建議，整體數據將匿名處理僅做調查分析用，請放心作答。",
  },
];

export const footerTranslations = [
  {
    id: 1,
    english: "Language",
    chinese: "語言 ",
  },
  {
    id: 2,
    english:
      "This version of the website is directed towards english speakers .If you prefer Chinese please select the language above.",
    chinese: "此網站版本主要設計給中文為第一語言者，如果需要請在上方選擇語言。",
  },
  {
    id: 3,
    english: "Happiness",
    chinese: "幸福",
  },
];

export const menuDropdownTranslations = [
  {
    id: 1,
    english: "FEEDBACK",
    chinese: "意見回饋",
  },
  {
    id: 2,
    english: "SURVEY",
    chinese: "調查",
  },
  {
    id: 3,
    english: "CONTACT US",
    chinese: "聯繫我們",
  },
];

export const resultsTranslations = [
  {
    id: 1,
    english: "MY RESULTS",
    chinese: "我的結果",
  },
  {
    id: 2,
    english: "Positive:",
    chinese: "正向值:",
  },
  {
    id: 3,
    english: "Negative:",
    chinese: "負向值:",
  },
  {
    id: 4,
    english: "Start New Survey",
    chinese: "開始新測驗",
  },
  {
    id: 5,
    english:
      "You still haven't completed the survey! To receive some feedback first finish it.",
    chinese: "你還沒完成問卷調查！請先完成問卷以獲得反饋意見。",
  },
];
